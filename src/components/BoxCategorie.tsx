...
export function BoxCategorie(props: BoxCategorieProps) {
	return (
		<div className="h-52 w-48 rounded-lg border-2 border-roxo cursor-pointer transition-all" onClick={() => props.link && Router.push(props.link)}>
			...
			<div className="h-1/5 w-full flex items-center justify-center rounded-b-lg bg-slate-50 text-roxo">
				{props.content}
			</div>
		</div>
	)
